...
def generate_s3_output_key():
    ...
    return f"Raw/TMDB/JSON/{date_str}/dc-sci-fi-movies.json"

def fetch_dc_sci_fi_movies():
    all_movies = []
    page = 1 
    while True:
        discover_params = {
...
def fetch_movie_details(movie_id):
    try:
        ...
        for company in movie_details.get("production_companies", []):
            if company.get("id") == DC_STUDIO_ID: 
...
def lambda_handler(event, context):
    ...
    dc_sci_fi_movies = fetch_dc_sci_fi_movies()
    detailed_movies = []

    # Buscar os detalhes de cada filme
    for movie in dc_sci_fi_movies:
    ...
    if not detailed_movies:
        return {
            'statusCode': 400,
            'body': json.dumps("Nenhum filme da DC de sci-fi foi encontrado ou processado.")
        }
    ...
    return {
        'statusCode': 200,
        'body': json.dumps(f'{len(detailed_movies)} filmes da DC de Sci-Fi salvos com sucesso.')
    }
